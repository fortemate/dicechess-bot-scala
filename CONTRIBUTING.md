...
Thanks for helping build the Dice Chess Scala bot starter template. This repo follows the same
...
- SBT is the build tool; `dicechess-bot-runtime` is resolved from GitHub Packages, which
...
mise run check      # full validation: scalafmt + clean + tests (mirrors CI)
```

Other tasks: `mise run compile`, `mise run test`, `mise run format`, `mise run build:native`.
...
- **Branch naming** (enforced by CI): `<type>/<short-desc>`, optionally `<type>/<id>-<short-desc>` to
  ...
  `test` / `perf` (issueless). E.g. `feat/12-add-heuristic` or `refactor/update-runtime`.
